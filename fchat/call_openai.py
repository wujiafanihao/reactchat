...
async def call_openai(question: str) -> AsyncIterable[str]:
    ...
    try:
        ...
        # 设置初始消息
        initial_message = '{"role": "system", "content": "You is ChatGPT 3.5 turo,you was made of OPENAI"}\n'

        # 创建等待任务，调用模型生成响应
        coroutine = wait_done(llm.agenerate(messages=[[HumanMessage(content=initial_message), HumanMessage(content=question)]]), callback.done)
